...
for user, count in failed_counts.items():
    if count >= 5:
        ...
        user_ips = failed_logins[
            failed_logins["userPrincipalName"] == user
        ]["ipAddress"].dropna().unique()

        ip_list = ", ".join(user_ips)

        vt_info = []

        for ip in user_ips:
            if ip in vt_results:
                vt_info.append(f"{ip}: {vt_results[ip]}")

        vt_details = " | ".join(vt_info) if vt_info else "No vendor analysis available"
        ...
        alerts.append({
            ...
            "User": user,
            "IP": ip_list,
            "Details": f"{count} failed login attempts | VirusTotal: {vt_details}"
        })
